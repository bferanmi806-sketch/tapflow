...
- **Lean mode makes iOS simulators lighter while tapflow runs them.** Set `"agent": { "lean": true }` in `tapflow.config.json`, or answer On when `tapflow init` asks on a Mac, and the agent turns off background services an app under test does not use on each simulator it boots: Siri and Apple Intelligence background work, iCloud Keychain and backup, the Health app and HomeKit, photo analysis, Screen Time, iMessage and FaceTime, Continuity and telemetry among them. Measured on iOS 27, a simulator uses about a quarter less memory, roughly 0.5 GB, so a Mac holds more of them before it swaps. Wallpaper, widgets and the services apps call — push, StoreKit, CloudKit, HealthKit, the photo picker, universal links and more — stay on. The agent puts everything back when it shuts the simulator down, so the same simulator opened from Xcode is untouched. Off by default; iOS 18.5 or later; Android emulators are not covered yet. `TAPFLOW_LEAN=on|off` overrides the file, and `tapflow doctor ios` reports how many simulators are lean.
