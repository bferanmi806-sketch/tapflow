...
새 페이지는 frontmatter `title`·`description`이 필수다. 템플릿의 섹션 순서는 유지하되, 해당 없는 섹션은 생략한다.
...
`${{ }}` 표현식을 `run:` 블럭 안의 멀티라인 문자열에 직접 넣지 않는다. YAML 들여쓰기가 깨진다.
표현식은 `env:`에서 쉘 변수로 먼저 바인딩하고, `run:`에서는 `$VAR`로 쓴다.
