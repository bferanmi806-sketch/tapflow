...
2. 팀원의 이메일과 역할을 선택합니다:
   - **Admin** — 모든 작업이 가능합니다. 팀원 초대·역할 변경·삭제, 비밀번호 재설정, 워크스페이스 설정, **Settings → Tokens** 페이지는 Admin만 사용할 수 있습니다.
   - **Developer** — 앱을 추가·수정·삭제할 수 있습니다.
   - **QA**, **Viewer** — 아래의 공통 작업을 할 수 있습니다. 현재 서버는 두 역할을 구분하지 않습니다.

   빌드 업로드, 빌드 상태 변경, 댓글 작성, 세션 시작은 역할과 관계없이 로그인한 팀원 누구나 할 수 있습니다. Viewer도 읽기 전용이 아닙니다.
3. **Generate invite link**를 클릭합니다. 초대 링크가 다이얼로그에 표시되고 클립보드에 복사됩니다. SMTP가 설정되어 있으면 팀원에게 비밀번호 설정 링크가 담긴 초대 이메일도 발송됩니다.
...
App Center에서 빌드 행을 클릭하면 QA 세션 화면이 열립니다. **Select Mac**에서 Mac을 고르고 **Select device**에서 디바이스를 클릭하면 세션이 시작됩니다. 디바이스 화면이 실시간으로 브라우저에 스트리밍됩니다.
