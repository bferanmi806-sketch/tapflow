...
터널 플래그 없이 대화형 터미널에서 실행하면 터널 선택 화면이 표시됩니다. 터널을 고르지 않으면 스트리밍 성능(HTTPS)과 Lean mode도 묻고 답을 설정 파일의 `tls`와 `agent.lean`에 적습니다. 비대화형 환경에서 `--tunnel` 없이 실행하면 터널 없는 기본 설정 파일이 생성됩니다.
...
| `--tunnel <provider>` | — | `tailscale` 또는 `rathole`. `tapflow.config.json`에 `tunnel` 섹션이 없으면 오류로 멈춥니다. 실제로 띄우는 터널은 이 값과 관계없이 `tunnel` 섹션이 정하며 이 플래그 없이도 `tunnel` 섹션이 있으면 터널을 띄웁니다. |
...
릴레이가 메모리에 보관하는 최근 로그 항목을 출력합니다(기본값: 최근 100줄). 지금 이 버퍼에 기록되는 것은 자격 증명이 없어 거부된 WebSocket 연결뿐입니다. 릴레이의 전체 로그는 릴레이를 실행한 터미널에 출력됩니다.
