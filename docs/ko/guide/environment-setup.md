...
| 영역 | 항목 |
|------|------|
| 공통 | Node 버전, 포트 4000 사용 가능 여부 |
| iOS | Xcode, `xcrun simctl`, 사용 가능한 시뮬레이터, 네트워크 필터(설치 상태·버전), 네트워크 훅, Lean 모드 |
| Android | Android SDK, adb, build-tools (aapt), AVD, Lean 모드 |

각 항목은 **✓ 준비됨**, **⚠ 주의**, **✗ 설치 필요**로 표시됩니다. 기기가 실행 중인지는 보지 않습니다. 부팅은 대시보드에서 세션에 접속할 때 자동으로 이뤄지므로, 부팅할 수 있는 기기가 하나라도 있으면 통과로 간주합니다.

포트 검사는 설정 파일의 포트와 관계없이 항상 4000번을 봅니다. 기본 포트로 실행 중인 릴레이처럼 무언가 4000번 포트를 쓰고 있으면 이 항목이 실패하고 doctor가 종료 코드 `1`로 끝나므로 릴레이를 멈춘 상태에서 실행하거나 이 항목을 무시하세요.
...
setup은 한 번에 끝나도록 설계됐습니다. 설치가 필요한 단계에서는 대화형 터미널에서 동의를 구한 뒤 직접 실행합니다. 비대화형(CI 등) 환경에서는 설치 대신 필요한 명령을 안내합니다. 할 일이 남아 있으므로 종료 코드 `1`로 끝납니다.

### iOS

- **Homebrew**가 없으면 설치합니다.
...
- **시뮬레이터 런타임**이 없어 사용 가능한 기기가 하나도 없으면 런타임을 내려받습니다.
- **오디오 권한**: 시뮬레이터 오디오 캡처에 필요한 권한을 미리 요청합니다. 자세한 내용은 [오디오](/ko/guide/audio)를 참고하세요.
- **네트워크 필터**: 네트워크 제어에 쓰는 필터를 설치합니다. 자세한 내용은 [네트워크 제어](/ko/guide/network-control)를 참고하세요.
...
## 기기 부팅은 자동입니다 {#디바이스-부팅은-자동입니다}

setup은 부팅할 수 있는 기기와 AVD를 준비하는 데까지만 합니다. 실제 부팅은 팀원이 대시보드에서 QA 세션에 접속할 때 릴레이가 자동으로 처리하므로, setup 직후에 기기를 직접 띄울 필요는 없습니다.
